...
<div id="loader">
  ...
  {#if !hideProgress}
    <h2 id="progress" bind:this={pourcentElement}>
      <span
        style="translate: 0 {pourcent < 100
          ? 1
          : -Math.floor(pourcent / 100)}em;"
      >
        {#each Array(10)
          .fill(null)
          .map((_, i) => i) as i}
      ...
      <span style="translate: 0 -{Math.floor((pourcent % 100) / 10)}em;">
        {#each Array(10)
          .fill(null)
          .map((_, i) => i) as i}
          <span class="digit">{i}</span>
        {/each}</span
      >
      <span style="translate: 0 -{pourcent % 10}em;">
        {#each Array(10)
          .fill(null)
          .map((_, i) => i) as i}
          <span class="digit">{i}</span>
