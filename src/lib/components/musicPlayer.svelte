<script lang="ts">
  ...
  export const controls = {
    ...
    play(query?: number | string | Music | ((m: Music, i: number) => boolean)) {
          ...
          ? typeof query === "number"
            ...
            : typeof query === "string"
              ? list.find(
                  (m) =>
                    m.audioSrc === query ||
                    m.metadata.file === query ||
                    m.metadata.title === query
                )
      ...
      if (!music)
        throw new Error(
          `Music has not been found with query: ${query?.toString()}.`
        );

      if (music === "current") {
        if (!$current)
          throw new Error("Cannot resume music as it is undefined.");
      ...
      if ($current)
        return {
          ...
          index: list.findIndex(
            ({ audioSrc }) => audioSrc === $current?.music.audioSrc
          ),
        };
    },
    get status() {
          ...
          <label for="slider">
            {#key $current.player.ready}
              <input
                class="nodefault"
                type="range"
                bind:value={$current.player.e.currentTime}
                on:change={(e) => {
                  if ($current)
                    $current.player.e.currentTime = parseFloat(
