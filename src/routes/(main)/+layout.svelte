...
<script lang="ts">
    ...
    onMount(() => {
        ...
        backgroundScene.subscribe(async scene => {
            if(scene && scene.constructor.name === currentScene?.constructor.name) return;
            
            if(currentScene) Scene.stopAnimation(currentScene)
            currentScene= undefined
            sceneContainer?.children[0]?.remove();
            if(loader && loading) loader.animationEnded.subscribe(ended => {
                loading = !ended
            ...
            await scene.load();
            Scene.animate(scene);
            if("setBG" in scene && typeof scene.setBG === "function") scene.setBG($theme)
            loader?.loaded()
            loadingScene = false;
        })
