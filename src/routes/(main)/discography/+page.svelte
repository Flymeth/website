<script lang="ts">
  ...
  const musics = Array.from(data.musics).sort(
  ...
  onMount(() => {
    keepPlayerIfNoSong = !window.navigator.userAgent
      .toLowerCase()
      .includes("mobile");
  });
</script>
...
<svelte:head>
  ...
  <meta
    name="description"
    content="J'adore écouter de la musique! Un jour je me suis demandé si je pouvais en créer..."
  />
