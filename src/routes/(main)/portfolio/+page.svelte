<script lang="ts">
	...
	opennedProject.subscribe((projectID) => {
		if (browser) {
			const url = projectlist.find((p) => p.metadata.id === projectID)
				? new URL(`#${projectID}`, $page.url.href)
				: new URL($page.url.pathname, $page.url.origin);
	...
	onMount(() => {
		if ($opennedProject) {
			const { top, bottom, height } =
				projectContainer.getBoundingClientRect();
			window.scrollTo({
...
<Header>
	...
	<p>
		Ici sont répertoriés mes meilleures projets ainsi que des explications
		sur la réalisation de ceux-ci.
	</p>
</Header>

<main>
	<ul bind:this={projectContainer}>
		{#each [...projectlist].sort(() => Math.random() - 0.5) as project (project.metadata.id)}
			<li
				use:reveal={{
					disable: !!preOpenID,
				}}
